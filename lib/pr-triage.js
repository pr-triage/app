class PRTriage {
  ...
  async _getState() {
    ...
    if (reviews.length === 0) {
      ...
      const approvedReviews = reviews.filter(review => review.state === PRTriage.GH_REVIEW_STATE.APPROVE);
  ...
  async _getUniqueReviews() {
    const reviews = (await this.github.pullRequests.getReviews(this.config)).data || [];
    ...
    return this.pullRequest.labels
      .map(label => {
        ...
        for (const labelKey in filteredConfig) {
          ...
          if (configValue && configValue.name && label.name === configValue.name) {
            return labelKey;
          }
        }
      })
