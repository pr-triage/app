class PRTriage {
  ...
  async _getState() {
    ...
    const numRequestedReviews = await this._getRequestedNumberOfReviews();
    ...
    if (reviews.length === 0) {
      ...
      if (changeRequestedReviews.length > 0) {
        ...
        approvedReviews.length < numRequestedReviews
      ) {
        // Mark if partially approved if:
        // 1) Branch protections require more approvals
        //  - or -
        // 2) not everyone requested has approved
        // (2) might need to be configurable in the future
