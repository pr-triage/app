class PRTriage {
  ...
  async _check(state) {
    ...
    const labels = await this.github.issues
      .get({ owner, repo, issue_number })
      .then((res) => res.data.labels.flatMap((x) => x.name));

    this.logger.debug(labels);

    let definedLabelCount = 0;
    const definedLabels = Object.values(
      this._getFilteredConfigObjByRegex(/label_*/)
    ).flatMap((x) => x.name);
    for (let i = 0; i < labels.length; i++) {
      if (definedLabels.includes(labels[i])) {
        definedLabelCount++;
      }
    }

    if (
      state !== PRTriage.STATE.MERGED &&
      state !== PRTriage.STATE.WIP &&
      definedLabelCount === 0
    ) {
      throw new Error("Should have 1 defined label at least");
    }

    if (definedLabelCount > 1) {
      throw new Error("With extra labels.");
    }
  }
